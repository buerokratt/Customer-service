...
describe('SelectedChatToolbox component', () => {
  ...
  // it('should post event message when ask permission button is pressed', () => {
  //   ChatService.postEventMessage = jest.fn(() => Promise.resolve());
  //   render(
  //     <Provider store={testStore}>
  //       <ChatToolbox />
  //     </Provider>,
  //   );
  //   fireEvent.click(screen.getByRole('button', { name: 'chatToolbox.action.askPermission' }));
  //   expect(ChatService.postEventMessage).toHaveBeenCalledTimes(1);
  //   jest.restoreAllMocks();
  // });
  //
  // it('should send message with new event when ask permission button is pressed second time', () => {
